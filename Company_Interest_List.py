...
def menu():     # Displays the menu for the user to navigate through the app
    while True:
...
def add_company():    # Adds a company to the list of companies, if it doesn't already exist
...
def remove_company():   # Removes a company from the list of companies, if it already exists
    if len(companies) == 0:     # Checks that the list is empty before attempting to remove
        ...
        return
    company = input("Enter the company name to remove: ").strip()  
    if company == "":
        print("Company name cannot be empty!")
    ...
    for i in companies:  
        if i.lower() == company.lower():   
            companies.remove(i)    # Removes company from list
...
def display_companies():    # Displays all the companies in the list in a clear, numbered format
    if len(companies) == 0:    
